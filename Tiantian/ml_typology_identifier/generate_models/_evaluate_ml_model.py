...
def evaluate_ml_model(path_model_parameters_json, path_folder_sub_model, min_percentage=None, is_saved=None):
    """
    ...
    path_training_data = os.path.join(path_folder_sub_model, "Training")
    path_test_data = os.path.join(path_folder_sub_model, "Test")
    path_model_pkl = os.path.join(path_folder_sub_model, "model.pkl")
    ...
    # Result
    with open(os.path.join(path_model_parameters_json[0:len(path_model_parameters_json)-17], "eval_result.json"), "w") as out_file:
        json.dump(evaluation_dictionary, out_file, indent=4)
    # Saved
    if is_saved:
        with open(os.path.join(path_model_parameters_json[0:len(path_model_parameters_json)-17], f"{min_percentage}.json"), "w") as out_file:
            json.dump(evaluation_dictionary, out_file, indent=4)
