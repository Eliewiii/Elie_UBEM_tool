...
path_folder_sub_model_list = [] #todo
...
if __name__ == "__main__":  # mandatory for parallel processing, what is executed by the master should be under this
    ...
    dt = time()  # timer
    # begin Pool parallel
    for path_folder in path_folder_sub_model_list:
        clean_directory(os.path.join(path_folder,"training"))
        clean_directory(os.path.join(path_folder,"test"))

        p = Pool(nb_simultaneous_process)
        p.starmap(generate_sample_ml, [(building_type_input_list[i],type_data[i],path_folder) for i in range(nb_process)], chunksize=1)
        p.close()
        p.join()
