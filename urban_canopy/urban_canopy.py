...
class Urban_canopy(test.Other,test_2.Other):
    """
    ...
    def extract_gis_2D(self, path, unit):
        ...
        for building_number_shp in range(0, number_of_building_shp):
            ...
            if isinstance(footprint,
                          shapely.geometry.polygon.Polygon):  # if the building is made of 1 footprint (isinstance check if the type is correct)
                self.polygon_to_building(footprint, shape_file, building_number_shp, unit)
            elif isinstance(footprint,
                            shapely.geometry.multipolygon.MultiPolygon):  # if the building is made of multiple footprints
    ...
    def create_simulation_folder_buildings(self, path_folder_building_simulation):
        """ Generate the output simulation folder and the sub-folder of each simulated building """
        for building_id in self.building_to_simulate:
            path_dir_building = os.path.join(path_folder_building_simulation, self.building_dict[building_id].name)
            make_sub_folders(path_dir_building,["Building_object","Context_surfaces_json", "EnergyPlus_simulation", "HBjson_model", "Results"])

# todo : remove this version of the simulation folder
    def create_folder_simulation(self, path_folder_simulation, path_folder_context_hbjson,
                                 path_folder_building_simulation):
        """
