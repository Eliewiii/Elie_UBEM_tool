...
class UrbanCanopy:
    ...
    def add_list_of_buildings(self, building_id_list, building_obj_list):
        ...
        for building_id in building_id_list:
            building_obj = building_obj_list[building_id]
            # check if the building id is already in the urban canopy
            if building_id in self.building_dict.keys():
                logging.warning(f"The building id {building_id} is already in the urban canopy")
            else:
    ...
    def add_2d_gis(self, path_gis, building_id_key_gis= "idbinyan", unit = "m" ):
